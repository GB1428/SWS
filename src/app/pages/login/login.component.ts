      ...
      this.authService.usuarioCompleto$.subscribe(usuarioCompleto => {
        if (isAuthenticated) {
          ...
          if (usuarioCompleto.rol === "docente") {
            this.usuario = ''; // Limpiar el campo de usuario
            this.clave = ''; // Limpiar el campo de clave
            this.router.navigate(['/vista-profesor']); // Redirigir al usuario docente si el login es exitoso
          }
          else{
            this.usuario = ''; // Limpiar el campo de usuario
            this.clave = ''; // Limpiar el campo de clave
            this.router.navigate(['/vista-alumno']); // Redirigir al usuario alumno si el login es exitoso
          }
